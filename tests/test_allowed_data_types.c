...
// RUN: llvm-link $OUT_FILE.bc $OUT_FILE.externals.ok.bc -S -o  $OUT_FILE.linked.ll &&
// RUN: llvm-link $OUT_FILE.clamped.ll $OUT_FILE.externals.ok.bc -S -o  $OUT_FILE.clamped.linked.ll &&
// RUN: opt -O3 $OUT_FILE.clamped.linked.ll -S -o  $OUT_FILE.clamped.linked.o3.ll &&
// RUN: ( lli $OUT_FILE.clamped.linked.ll;
// RUN:   ( [ $? == 48 ] && echo "OK: running code did result 48 as expected.") ||
